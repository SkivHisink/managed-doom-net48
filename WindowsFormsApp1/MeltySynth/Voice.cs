...
namespace MeltySynth
{
    internal sealed class Voice
    {
        ...
        public void Start(RegionPair region, int channel, int key, int velocity)
        {
            ...
            instrumentPan = DoomGame.Clamp(region.Pan, -50F, 50F);
        ...
        public bool Process()
        {
            ...
            var angle = (Math.PI / 200F) * (channelInfo.Pan + instrumentPan + 50F);
            if (angle <= 0F)
            {
            ...
            currentReverbSend = DoomGame.Clamp(channelInfo.ReverbSend + instrumentReverb, 0F, 1F);
            currentChorusSend = DoomGame.Clamp(channelInfo.ChorusSend + instrumentChorus, 0F, 1F);
