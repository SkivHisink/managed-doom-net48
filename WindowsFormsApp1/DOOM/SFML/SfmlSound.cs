    ...
    public sealed class SfmlSound : ISound, IDisposable
    {
        ...
        public SfmlSound(Config config, Wad wad)
        {
            try
            {
                ...
                config.audio_soundvolume = DoomGame.Clamp(config.audio_soundvolume, 0, MaxVolume);
        ...
        public void StartSound(Mobj mobj, Sfx sfx, SfxType type, int volume)
        {
            ...
            {
                priority = amplitudes[(int)sfx] * GetDistanceDecay((float)dist) * volume;
            }
        ...
        private void SetParam(Sound sound, ChannelInfo info)
        {
            ...
            else
            {
                ...
                {
                    var dist = Math.Sqrt(x * x + y * y);
                    var angle = Math.Atan2(y, x) - (float)listener.Angle.ToRadian() + Math.PI / 2;
                    sound.Position = new Vector3f((float)Math.Cos(angle), (float)Math.Sin(angle), 0);
                    sound.Volume = masterVolumeDecay * GetDistanceDecay((float)dist) * info.Volume;
                }
            }
        }
