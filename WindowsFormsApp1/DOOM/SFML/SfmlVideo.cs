    ...
    public sealed class SfmlVideo : IVideo, IDisposable
    {
        ...
        public SfmlVideo(Config config, GameContent content, RenderWindow window)
        {
            try
            {
                ...
                config.video_gamescreensize = DoomGame.Clamp(config.video_gamescreensize, 0, MaxWindowSize);
                config.video_gammacorrection = DoomGame.Clamp(config.video_gammacorrection, 0, MaxGammaCorrectionLevel);
