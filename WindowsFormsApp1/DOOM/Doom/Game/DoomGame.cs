...
namespace ManagedDoom
{
    ...
    public sealed class DoomGame
    {
		...
		{
			options.Skill = (GameSkill)DoomGame.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = DoomGame.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = DoomGame.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = DoomGame.Clamp(map, 1, 9);
			}
