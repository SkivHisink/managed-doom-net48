...
namespace ManagedDoom.Video
{
    public sealed class DrawScreen
    {
        ...
        public void DrawLine(float x1, float y1, float x2, float y2, int color)
        {
            ...
            if (accept)
            {
                var bx1 = DoomGame.Clamp((int)x1, 0, width - 1);
                var by1 = DoomGame.Clamp((int)y1, 0, height - 1);
                var bx2 = DoomGame.Clamp((int)x2, 0, width - 1);
                var by2 = DoomGame.Clamp((int)y2, 0, height - 1);
